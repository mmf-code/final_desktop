...
def create_simple_comparison_plot(file_dict):
    ...
    plt.tight_layout()
    plt.savefig('figure1_basic_performance.png', dpi=300, bbox_inches='tight')
    plt.show()
    ...
    for i, scenario in enumerate(scenarios):
        ...
        if 'ErrorY0' in df.columns:
            avg_error_y = (df['ErrorY0'] + df['ErrorY1'] + df['ErrorY2']) / 3
    ...
    for i, scenario in enumerate(scenarios):
        ...
        ax7.plot(data[scenario]['Time'], error_rate_smooth, color=colors[i],
                 label=scenario, linewidth=2)
    ...
    for i, scenario in enumerate(scenarios):
        ...
        if 'centroid_x' in metrics and 'target_centroid_x' in metrics:
            ...
            ax8.plot(data[scenario]['Time'], centroid_error, color=colors[i],
                     label=scenario, linewidth=2)
    ...
    plt.tight_layout()
    plt.savefig('figure3_formation_control.png', dpi=300, bbox_inches='tight')
    plt.show()
...
def main():
    ...
    if len(file_dict) < 2:
        ...
        print("Available files should have pattern: Kp3.100_Ki0.400_Kd2.200")
        return
