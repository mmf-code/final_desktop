...
static bool loadPIDParams(const std::string &file, PIDParams &params) {
  ...
  while (std::getline(in, line)) {
    ...
    if (line.rfind("kp:", 0) == 0)
      params.kp = std::stod(trim(line.substr(3)));
    else if (line.rfind("ki:", 0) == 0)
      params.ki = std::stod(trim(line.substr(3)));
    else if (line.rfind("kd:", 0) == 0)
      params.kd = std::stod(trim(line.substr(3)));
    else if (line.rfind("output_limits:", 0) == 0) {
      // read next lines for min/max
      std::string sub;
      if (std::getline(in, sub)) {
        sub = trim(sub);
        if (sub.rfind("min:", 0) == 0)
          params.output_min = std::stod(trim(sub.substr(4)));
      }
      if (std::getline(in, sub)) {
        sub = trim(sub);
        if (sub.rfind("max:", 0) == 0)
          params.output_max = std::stod(trim(sub.substr(4)));
      }
    }
  }
  return true;
...
static void applyFuzzyParams(agent_control_pkg::GT2FuzzyLogicSystem &fls,
                             const FuzzyParams &fp) {
  ...
  for (const auto &varPair : fp.sets) {
    ...
    for (const auto &setPair : varPair.second) {
      ...
      fls.addFuzzySetToVariable(
          var, setPair.first,
          FOU{fou.l1, fou.l2, fou.l3, fou.u1, fou.u2, fou.u3});
    }
  }
...
struct FakeDrone {
  ...
  void update(double accel_cmd_x, double accel_cmd_y, double dt,
              ...
              double external_force_y = 0.0) { // Add external forces
    // Assume mass = 1 for simplicity, so force = acceleration
    velocity_x +=
        (accel_cmd_x + external_force_x) * dt; // Add external force component
    velocity_y +=
        (accel_cmd_y + external_force_y) * dt; // Add external force component
...
int main(int argc, char **argv) {
  ...
  const bool ZN_TUNING_ACTIVE =
      true; // SET TO false FOR NORMAL RUN WITH Z-N GAINS
  ...
  // *** FLS INSTANCES ***
  std::vector<agent_control_pkg::GT2FuzzyLogicSystem> fls_x_controllers(
      NUM_DRONES);
  std::vector<agent_control_pkg::GT2FuzzyLogicSystem> fls_y_controllers(
      NUM_DRONES);
  // *** END FLS INSTANCES ***
  ...
  if (ZN_TUNING_ACTIVE) {
    std::cout << "!!!!!!!! ZIEGLER-NICHOLS Ku/Pu FINDING MODE ACTIVE !!!!!!!!"
              << std::endl;
    std::cout << "Testing with Kp = " << ZN_KP_TEST_VALUE << ", Ki = 0, Kd = 0"
              << std::endl;
    std::cout << "Observe Drone 0 X-axis for sustained oscillations."
              << std::endl;
    kp = ZN_KP_TEST_VALUE;
  ...
  std::string controller_type_log_msg =
      USE_FLS ? "PID+FLS Controller" : "PID-Only Controller";
  std::string timestamp = getCurrentTimestamp();
  std::string csv_file_name =
      config.csv_prefix + "_" + timestamp + (USE_FLS ? "_FLS" : "_NOFLS") +
      (config.wind_enabled ? "_WIND_ON" : "_WIND_OFF") + ".csv";
  std::string metrics_file_name = std::string("performance_metrics_") +
                                  timestamp + (USE_FLS ? "_fls" : "_pid") +
                                  (config.wind_enabled ? "_wind" : "_nowind") +
                                  ".txt";

  std::filesystem::create_directories(config.output_directory);
  std::filesystem::path csv_path =
      std::filesystem::path(config.output_directory) / csv_file_name;
  std::filesystem::path metrics_path =
      std::filesystem::path(config.output_directory) / metrics_file_name;
  ...
  for (int i = 0; i < NUM_DRONES; ++i) {
    csv_file << ",TargetX" << i << ",CurrentX" << i << ",ErrorX" << i
             ...
             << ",PTermX" << i << ",ITermX" << i << ",DTermX"
             << i // <<< X-axis PID Term headers
             << ",TargetY" << i << ",CurrentY" << i << ",ErrorY" << i
             << ",PIDOutY" << i << ",FLSCorrY" << i << ",FinalCmdY" << i
             << ",PTermY" << i << ",ITermY" << i << ",DTermY"
             << i; // <<< Y-axis PID Term headers
  }
  csv_file << ",SimWindX,SimWindY"
           << std::endl; // Make sure this matches what your Python script
                         // expects for wind columns

  // Open metrics file
  std::ofstream metrics_file;
  if (!ZN_TUNING_ACTIVE) { // Only create metrics file if not in P-only ZN
                           // step-by-step tuning
    metrics_file.open(metrics_path);
    if (!metrics_file.is_open()) {
      std::cerr << "Error: Could not open metrics file: " << metrics_path
                << std::endl;
  ...
  std::vector<bool> phase_triggered(
      MAX_PHASES + 1, false); // To ensure each phase change happens once
  ...
  std::vector<std::vector<double>> initial_values_x_per_phase(
      num_metric_phases, std::vector<double>(NUM_DRONES));
  std::vector<std::vector<double>> initial_values_y_per_phase(
      num_metric_phases, std::vector<double>(NUM_DRONES));
  std::vector<std::vector<double>> target_values_x_per_phase(
      num_metric_phases, std::vector<double>(NUM_DRONES));
  std::vector<std::vector<double>> target_values_y_per_phase(
      num_metric_phases, std::vector<double>(NUM_DRONES));
  std::vector<std::vector<bool>> in_settling_band_x(
      num_metric_phases, std::vector<bool>(NUM_DRONES, false));
  std::vector<std::vector<bool>> in_settling_band_y(
      num_metric_phases, std::vector<bool>(NUM_DRONES, false));
  std::vector<std::vector<double>> time_entered_settling_x(
      num_metric_phases, std::vector<double>(NUM_DRONES, -1.0));
  std::vector<std::vector<double>> time_entered_settling_y(
      num_metric_phases, std::vector<double>(NUM_DRONES, -1.0));
  ...
  formation_offsets[1] = {-side_length / 2.0,
                          -(sqrt(3.0) / 2.0) * side_length * (1.0 / 3.0)};
  formation_offsets[2] = {side_length / 2.0,
                          -(sqrt(3.0) / 2.0) * side_length * (1.0 / 3.0)};

  for (int i = 0; i < NUM_DRONES; ++i) {
    target_values_x_per_phase[0][i] =
        formation_center_x + formation_offsets[i].first;
    target_values_y_per_phase[0][i] =
        formation_center_y + formation_offsets[i].second;
    pid_x_controllers[i].setSetpoint(target_values_x_per_phase[0][i]);
    pid_y_controllers[i].setSetpoint(target_values_y_per_phase[0][i]);
  }
  ...
  double simulation_time =
      config.total_time; // Extended simulation time for more phases
  ...
  for (double time_now = 0.0; time_now <= simulation_time; time_now += dt) {
    ...
    if (config.wind_enabled && !ZN_TUNING_ACTIVE) {
      for (const auto &wp : config.wind_phases) {
        if (wp.phase_number != current_phase)
          continue;
    ...
    for (int p = MAX_PHASES; p >= 1; --p) {
      if (time_now >= phase_start_times[p - 1]) {
        desired_phase = p;
        break;
      }
    }
    if (desired_phase != current_phase) {
      ...
      for (int i = 0; i < NUM_DRONES; ++i) {
        ...
        target_values_x_per_phase[metric_idx][i] =
            formation_center_x + formation_offsets[i].first;
        target_values_y_per_phase[metric_idx][i] =
            formation_center_y + formation_offsets[i].second;
        pid_x_controllers[i].setSetpoint(
            target_values_x_per_phase[metric_idx][i]);
        pid_y_controllers[i].setSetpoint(
            target_values_y_per_phase[metric_idx][i]);
        drone_metrics_x[i][metric_idx].reset(
            initial_values_x_per_phase[metric_idx][i]);
        drone_metrics_y[i][metric_idx].reset(
            initial_values_y_per_phase[metric_idx][i]);
    ...
    for (int i = 0; i < NUM_DRONES; ++i) {
      ...
      // Calculate dError
      double d_error_x =
          (dt > 1e-9) ? (error_x - drones[i].prev_error_x) / dt : 0.0;
      double d_error_y =
          (dt > 1e-9) ? (error_y - drones[i].prev_error_y) / dt : 0.0;

      // PID Calculation
      agent_control_pkg::PIDController::PIDTerms pid_terms_x =
          pid_x_controllers[i].calculate_with_terms(drones[i].position_x, dt);
      agent_control_pkg::PIDController::PIDTerms pid_terms_y =
      ...
      if (USE_FLS) { // Correctly uses the effective USE_FLS flag
        fls_correction_x = fls_x_controllers[i].calculateOutput(
            error_x, d_error_x, simulated_wind_x);
        fls_correction_y = fls_y_controllers[i].calculateOutput(
            error_y, d_error_y, simulated_wind_y);
      }
      // *** END FLS Calculation ***

      // *** Combine PID + FLS and Clamp ***
      double final_cmd_x = clamp(pid_terms_x.total_output + fls_correction_x,
                                 output_min, output_max);
      double final_cmd_y = clamp(pid_terms_y.total_output + fls_correction_y,
                                 output_min, output_max);
      // *** END Combine & Clamp ***
      ...
      if (metric_idx < 0 || metric_idx >= num_metric_phases) {
        ...
        if (current_phase == 1)
          metric_idx = 0;
        // else std::cerr << "Error: Invalid metric_idx " << metric_idx << " at
        // time " << time_now << std::endl;
      }

      PerformanceMetrics &current_drone_metric_x =
          drone_metrics_x[i][metric_idx];
      PerformanceMetrics &current_drone_metric_y =
          drone_metrics_y[i][metric_idx];
      ...
      if (settling_tol_x < 1e-3)
        settling_tol_x = 1e-3; // Minimum tolerance if target is near zero
      double settling_tol_y = std::abs(current_target_y * SETTLING_PERCENTAGE);
      if (settling_tol_y < 1e-3)
        settling_tol_y = 1e-3;
      ...
      if (std::abs(drones[i].position_x - current_target_x) <= settling_tol_x) {
        ...
        if (current_drone_metric_x.settling_time_2percent < 0.0 &&
            in_settling_band_x[metric_idx][i]) {
          current_drone_metric_x.settling_time_2percent =
              time_entered_settling_x[metric_idx][i];
        }
      } else {
        if (in_settling_band_x[metric_idx][i]) { // Left the band after entering
          current_drone_metric_x.settling_time_2percent =
              -1.0; // Reset settling time
        }
        in_settling_band_x[metric_idx][i] = false;
      ...
      if (std::abs(drones[i].position_y - current_target_y) <= settling_tol_y) {
        ...
        if (current_drone_metric_y.settling_time_2percent < 0.0 &&
            in_settling_band_y[metric_idx][i]) {
          current_drone_metric_y.settling_time_2percent =
              time_entered_settling_y[metric_idx][i];
        }
      } else {
        if (in_settling_band_y[metric_idx][i]) {
      ...
      csv_file << "," << pid_x_controllers[i].getSetpoint() << ","
               ...
               << pid_terms_x.total_output << "," << fls_correction_x << ","
               << final_cmd_x << "," << pid_terms_x.p << "," << pid_terms_x.i
               << "," << pid_terms_x.d << ","
               << pid_y_controllers[i].getSetpoint() << ","
               << drones[i].position_y << "," << error_y << ","
               << pid_terms_y.total_output << "," << fls_correction_y << ","
               << final_cmd_y << "," << pid_terms_y.p << "," << pid_terms_y.i
               << "," << pid_terms_y.d;

      if (i == 0 && static_cast<int>(time_now * 1000) % 1000 == 0) {
        std::cout << "T=" << std::fixed << std::setprecision(1) << time_now
                  << std::fixed << std::setprecision(3)
                  << " D0_X:" << drones[0].position_x
                  << " D0_Y:" << drones[0].position_y << " ErrX:" << error_x
                  << " ErrY:" << error_y << " FLS_X:" << fls_correction_x
                  << " FLS_Y:" << fls_correction_y
                  << " WindX:" << simulated_wind_x
                  << " WindY:" << simulated_wind_y << std::endl;
      }
    }
  ...
  oss_config << std::fixed << std::setprecision(3) << controller_type_log_msg
  ...
  for (int phase_idx_loop = 0; phase_idx_loop < num_metric_phases;
       ++phase_idx_loop) {
    if (!phase_triggered[phase_idx_loop + 1] && phase_idx_loop > 0)
      continue; // Skip phases not triggered, except phase 1 (idx 0)
    if (phase_idx_loop == 0 && !phase_triggered[1])
      phase_triggered[1] = true; // Phase 1 always considered "triggered"

    for (int i = 0; i < NUM_DRONES; ++i) {
      // X-axis
      if (drone_metrics_x[i][phase_idx_loop].settling_time_2percent < 0 &&
          in_settling_band_x[phase_idx_loop][i]) {
        drone_metrics_x[i][phase_idx_loop].settling_time_2percent =
            time_entered_settling_x[phase_idx_loop][i];
      }
      // Y-axis
      if (drone_metrics_y[i][phase_idx_loop].settling_time_2percent < 0 &&
          in_settling_band_y[phase_idx_loop][i]) {
        drone_metrics_y[i][phase_idx_loop].settling_time_2percent =
            time_entered_settling_y[phase_idx_loop][i];
      }
    }
  }

  for (int phase_idx = 0; phase_idx < num_metric_phases; ++phase_idx) {
    if (!phase_triggered[phase_idx + 1] && phase_idx > 0)
      continue;
    if (phase_idx == 0 && !phase_triggered[1])
    ...
    for (int i = 0; i < NUM_DRONES; ++i) {
      ...
      // Calculate overshoot percentage if peak exists and target is different
      // from initial
      if (std::abs(target_x - initial_x) > 1e-6) { // Avoid division by zero
        if (target_x > initial_x) { // Moving in positive direction
          mets_x.overshoot_percent =
              ((mets_x.peak_value - target_x) / (target_x - initial_x)) * 100.0;
        } else { // Moving in negative direction
          mets_x.overshoot_percent =
              ((target_x - mets_x.peak_value) / (initial_x - target_x)) * 100.0;
        }
        if (mets_x.peak_value == initial_x && target_x != initial_x)
          mets_x.overshoot_percent = 0; // No movement towards target yet
        if (mets_x.overshoot_percent < 0)
          mets_x.overshoot_percent = 0; // No overshoot if it didn't pass target
      } else {
        mets_x.overshoot_percent =
            0.0; // No change in setpoint, so no overshoot by definition
      }

      if (std::abs(target_y - initial_y) > 1e-6) {
        if (target_y > initial_y) {
          mets_y.overshoot_percent =
              ((mets_y.peak_value - target_y) / (target_y - initial_y)) * 100.0;
        } else {
          mets_y.overshoot_percent =
              ((target_y - mets_y.peak_value) / (initial_y - target_y)) * 100.0;
        }
        if (mets_y.peak_value == initial_y && target_y != initial_y)
          mets_y.overshoot_percent = 0;
        if (mets_y.overshoot_percent < 0)
          mets_y.overshoot_percent = 0;
      } else {
      ...
      std::stringstream ss;
      ss << std::fixed << std::setprecision(3) << "Drone " << i
         << " Metrics (Target X:" << target_x << ", Y:" << target_y << "):\n"
         << "  X-axis:\n"
         << "    Peak Value: " << mets_x.peak_value
         << " at t=" << mets_x.peak_time << "s\n"
         << "    Overshoot: " << mets_x.overshoot_percent << "%\n"
         << "    Settling Time (2%): "
         << (mets_x.settling_time_2percent >= 0
                 ? std::to_string(mets_x.settling_time_2percent) + "s"
                 : "Did not settle")
         << "\n"
         << "  Y-axis:\n"
         << "    Peak Value: " << mets_y.peak_value
         << " at t=" << mets_y.peak_time << "s\n"
         << "    Overshoot: " << mets_y.overshoot_percent << "%\n"
         << "    Settling Time (2%): "
         << (mets_y.settling_time_2percent >= 0
                 ? std::to_string(mets_y.settling_time_2percent) + "s"
                 : "Did not settle")
         << "\n";
