...
class TestResultsAnalyzer:
    ...
    def load_test_results(self, base_dir="simulation_outputs"):
        ...
        for pattern in patterns:
            ...
            if files:
                ...
                test_name = pattern.split('_')[0] + (
                    '_' + pattern.split('_')[1] if
                    'Only' in pattern or 'Wind' in pattern or
                    'System' in pattern else ''
                )
        ...
        if not self.results:
            print("⚠️ No test results found. Make sure you've run the "
                  "systematic tests first.")
            return False
        ...
        for test_name, df in self.results.items():
            ...
            for drone_id in df['drone_id'].unique():
                ...
                target_distance = np.sqrt(
                    drone_data['target_x'].iloc[-1]**2 +
                    drone_data['target_y'].iloc[-1]**2
                )
                max_distance = np.sqrt(
                    drone_data['position_x']**2 +
                    drone_data['position_y']**2
                ).max()
                overshoot = max(
                    0, (max_distance - target_distance) /
                    target_distance * 100
                )
                ...
                metrics[f'drone_{drone_id}'] = {
                    ...
                    'settling_time_2pct': (settling_time if settling_time
                                           else drone_data['time'].max()),
            ...
            all_overshoot = [
                metrics[f'drone_{i}']['overshoot_percent'] for i in range(3)
            ]
            all_settling = [
                metrics[f'drone_{i}']['settling_time_2pct'] for i in range(3)
            ]
            all_steady_error = [
                metrics[f'drone_{i}']['steady_state_error'] for i in range(3)
            ]
    ...
    def generate_comparison_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write("-" * 35 + "\n")
            f.write(f"{'Test':<20} {'Avg Overshoot':<15} "
                    f"{'Avg Settling':<15} {'Avg SS Error':<15}\n")
            f.write("-" * 65 + "\n")

            baseline_metrics = None
            for test_name, metrics in self.metrics_summary.items():
                overall = metrics['overall']
                f.write(f"{test_name:<20} {overall['avg_overshoot']:<15.2f} "
                        f"{overall['avg_settling_time']:<15.2f} "
                        f"{overall['avg_steady_error']:<15.4f}\n")
            ...
            if baseline_metrics:
                ...
                f.write("-" * 35 + "\n")
                f.write(f"{'Test':<20} {'Overshoot':<15} "
                        f"{'Settling':<15} {'SS Error':<15}\n")
                f.write("-" * 65 + "\n")
                ...
                for test_name, metrics in self.metrics_summary.items():
                    ...
                    overshoot_imp = (
                        (baseline_metrics['avg_overshoot'] -
                         overall['avg_overshoot']) /
                        baseline_metrics['avg_overshoot'] * 100
                    )
                    settling_imp = (
                        (baseline_metrics['avg_settling_time'] -
                         overall['avg_settling_time']) /
                        baseline_metrics['avg_settling_time'] * 100
                    )
                    error_imp = (
                        (baseline_metrics['avg_steady_error'] -
                         overall['avg_steady_error']) /
                        baseline_metrics['avg_steady_error'] * 100
                    )

                    f.write(f"{test_name:<20} {overshoot_imp:<15.1f} "
                            f"{settling_imp:<15.1f} {error_imp:<15.1f}\n")
            ...
            if baseline_metrics and 'FLS_Only' in self.metrics_summary:
                ...
                overshoot_imp = (
                    (baseline_metrics['avg_overshoot'] -
                     fls_metrics['avg_overshoot']) /
                    baseline_metrics['avg_overshoot'] * 100
                )
                f.write(f"FLS Effectiveness: {overshoot_imp:.1f}% "
            ...
            if baseline_metrics and 'Wind_Only' in self.metrics_summary:
                ...
                settling_deg = (
                    (wind_metrics['avg_settling_time'] -
                     baseline_metrics['avg_settling_time']) /
                    baseline_metrics['avg_settling_time'] * 100
                )
                f.write(f"Wind Impact: {settling_deg:.1f}% increase in "
                        f"settling time\n")
            ...
            if baseline_metrics and 'FF_Only' in self.metrics_summary:
                ...
                error_imp = (
                    (baseline_metrics['avg_steady_error'] -
                     ff_metrics['avg_steady_error']) /
                    baseline_metrics['avg_steady_error'] * 100
                )
                f.write(f"Feed-Forward Benefit: {error_imp:.1f}% "
                        f"steady-state error reduction\n")
    ...
    def create_comparison_plots(self):
        ...
        fig.suptitle('Systematic Testing Comparison',
                     fontsize=16, fontweight='bold')

        # Plot 1: Average Overshoot
        overshots = [
            self.metrics_summary[test]['overall']['avg_overshoot']
        ...
        settling_times = [
            self.metrics_summary[test]['overall']['avg_settling_time']
            for test in test_names
        ]
        axes[0, 1].bar(test_names, settling_times,
        ...
        ss_errors = [
            self.metrics_summary[test]['overall']['avg_steady_error']
            for test in test_names
        ]
        axes[1, 0].bar(test_names, ss_errors,
        ...
        axes[1, 1].bar(test_names, performance_score, color='gold',
                       alpha=0.7)
...
if __name__ == "__main__":
    ...
    if analyzer.run_full_analysis():
        print("\n✨ Use the generated plots and report for your final "
              "analysis!")
    else:
